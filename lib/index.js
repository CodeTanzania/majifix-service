 ...
 * @version 0.1.0
 */
const SlaSchema = mongooseCommon.createSubSchema({
  /**
   * @name ttr
 ...
 * @version 0.1.0
 */
const FlagsSchema = mongooseCommon.createSubSchema({
  /**
   * @name external
...
const SCHEMA_OPTIONS = { collection: majifixCommon.COLLECTION_NAME_SERVICE };
const INDEX_UNIQUE = {
  jurisdiction: 1,
  code: 1,
 ...
 * @public
 */
const ServiceSchema = mongooseCommon.createSchema(
  {
    /**
     * @name jurisdiction
    ...
    group: {
      type: mongooseCommon.ObjectId,
      ref: majifixServiceGroup.ServiceGroup.MODEL_NAME,
      required: true,
      exists: { refresh: true, select: majifixServiceGroup.ServiceGroup.OPTION_SELECT },
    ...
    type: {
      ...
      // required: true,
      exists: { refresh: true, select: predefine.Predefine.OPTION_SELECT },
      autopopulate: predefine.Predefine.OPTION_AUTOPOPULATE,
      index: true,
    },
...
ServiceSchema.methods.beforeDelete = function beforeDelete(done) {
  ...
  const dependencies = [majifixCommon.MODEL_NAME_SERVICEREQUEST];

  // path to check
  const path = majifixCommon.PATH_NAME_SERVICE;
...
/* static constants */
ServiceSchema.statics.MODEL_NAME = majifixCommon.MODEL_NAME_SERVICE;
...
ServiceSchema.statics.findDefault = done => {
  // refs
  const Service = mongooseCommon.model(majifixCommon.MODEL_NAME_SERVICE);
...
const Service = mongooseCommon.model(majifixCommon.MODEL_NAME_SERVICE, ServiceSchema);
