 ...
 * @private
 */
const ServiceSchema = new mongooseCommon.Schema(
  {
    /**
     * @name jurisdiction
  ...
  async.parallel(
    {
      ...
      type: function preloadServiceType(next) {
        ...
        const criteria = {
          _id: typeId,
          namespace: majifixCommon.PREDEFINE_NAMESPACE_SERVICETYPE,
          bucket: majifixCommon.PREDEFINE_BUCKET_SERVICETYPE,
        };

        // prefetch existing type
        if (typeId) {
          predefine.Predefine.getOneOrDefault(
            criteria,
...
ServiceSchema.methods.afterPost = function afterPost(done) {
  ...
  const type = _.merge(
    ...
    predefine.Predefine.OPTION_AUTOPOPULATE
  );
  this.populate(type);
...
const Service = mongooseCommon.model(SERVICE_MODEL_NAME, ServiceSchema);
